...
apt-get install -y docker-engine=1.9.0-0~trusty build-essential g++ libkrb5-dev libfontconfig
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # define some helpful bash aliases
  config.vm.provision "file", source: "./dev/bash_aliases", destination: ".bash_aliases"
  config.vm.provision "file", source: "./dev/docker-compose.yml", destination: "docker-compose.yml"

  config.vm.provision "shell", inline: $script
  config.vm.provision "shell", path: "./dev/build.sh"
